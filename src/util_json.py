...
from util_json_graficos import JsonAnaliseGraficos, traduzir_rotulos, _TRADUCOES
...
class JsonAnaliseDataFrame():
    ...
    def __init__(self, 
        ...
        self._lang = lang if lang in ('pt', 'en') else 'pt'  # Idioma dos gráficos
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # DELEGAÇÃO DE GRÁFICOS → JsonAnaliseGraficos (util_json_graficos.py)
    # ═════════════════════════════════════════════════════════════════════════
    
    @property
    def _graficos(self):
        """Instância lazy de JsonAnaliseGraficos para delegação de métodos de gráficos."""
        if not hasattr(self, '_graficos_instance'):
            self._graficos_instance = JsonAnaliseGraficos(
                dados_analise=self.dados_analise,
                rotulos=self.rotulos,
                pasta_analises=self.pasta_analises,
    ...
    def _consolidar_graficos_relatorio(self, pasta_saida: str):
        ...
        return self._graficos._consolidar_graficos_relatorio(pasta_saida)
    
    def gerar_graficos_metricas(self, arquivo_excel: str = None, pasta_saida: str = None, 
                                paleta: str = 'Cividis') -> list:
        """Delega para JsonAnaliseGraficos."""
        return self._graficos.gerar_graficos_metricas(arquivo_excel, pasta_saida, paleta)
    ...
    def gerar_graficos_observabilidade(self, arquivo_excel: str = None, pasta_saida: str = None,
                                       paleta: str = 'Cividis') -> list:
        """Delega para JsonAnaliseGraficos."""
        return self._graficos.gerar_graficos_observabilidade(arquivo_excel, pasta_saida, paleta)
