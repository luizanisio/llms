...
7. **Derivação do inventário de rótulos:** `inventario_tags()` levanta os 89 tipos semânticos (STY) presentes no **corpus inteiro**, com a frequência de cada um, e os injeta no prompt. O inventário sai do corpus, e não de um split, para que o prompt seja uma propriedade do corpus fixada antes de existir qualquer divisão: o protocolo A (zero-shot) é `baseline` da comparação do passo 06, e num desenho pareado ele precisa receber o mesmo estímulo que B, C e D*. Quatro STYs só ocorrem no teste (`Physical Object`, `Behavior`, `Regulation or Law`, `Social Behavior` — 5 de 8.699 anotações de teste, 0,06%): listá-los é vazamento de metadado, não de rótulo por instância, e está declarado no README §3. A cobertura do inventário é 1,0.
...
9. **Exportação final:** `exportar()` grava em `dados/` o dataset consolidado `semclinbr.parquet` (colunas `id`, `texto`, `resposta`, `prompt` e as extras estruturais; a divisão não é coluna do parquet, vem do arquivo do passo 03), o `prompt_semclinbr.txt` com o inventário já injetado e o `inventario_semclinbr.csv` com `rotulo,frequencia_corpus`. Junto com o parquet, é gerado `semclinbr.md` — o relatório de qualidade das anotações. O prompt é gravado junto com os dados de propósito: o inventário é derivado do corpus, então sem esse arquivo o experimento não é reprodutível.
...
| Rótulos no prompt (derivados do corpus) | 89 |
