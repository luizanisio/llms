...
6. **Exportação final:** `exportar()` grava em `dados/` o dataset consolidado `semclinbr.parquet` (colunas `id`, `texto`, `split`, `resposta`, `prompt` e as extras estruturais), o `divisao_semclinbr.csv` no formato `id_arquivo,alvo` que o framework espera em `arquivo_referencia`, o `prompt_semclinbr.txt` com o inventário já injetado e o `inventario_semclinbr.csv` com `rotulo,frequencia_treino`. O prompt é gravado junto com os dados de propósito: o inventário é derivado do corpus, então sem esse arquivo o experimento não é reprodutível.
